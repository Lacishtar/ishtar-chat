...
/**
 * ThemePresetSection — preset picker that lives at the top of the Inspector panel.
 *
 * Responsibilities (UI only):
 *   - Load the preset list via api.getPresetList()
 *   - Filter by search keyword
 *   - Apply a selected preset via api.applyPreset()
...
export default function ThemePresetSection({ api, onApplied, resetPreset }) {
  ...
  async function handleApply() {
    ...
    setApplying(true);
    try {
      const result = await api.applyPreset(selectedId);
      if (result?.ok) {
    ...
    <div className="flex flex-col gap-2.5 rounded-xl border border-focusAccent/30 bg-panelAlt/60 p-3">
      ...
      {selectedId && (() => {
        ...
        return p?.description ? (
          <p className="text-[10px] text-inkMuted leading-relaxed">{p.description}</p>
        ) : null;
      })()}
